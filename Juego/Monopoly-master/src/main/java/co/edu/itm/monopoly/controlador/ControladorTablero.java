...
public class ControladorTablero {
	...
	public ControladorTablero(ControladorInfo ctrlInfo, ControladorOpc ctrlOpc){
		...
		jugadorActivo = 0;
		numeroJugadores = 4;
		imgTablero = new JLabel(new ImageIcon("src/main/java/img/tablero.png"));
		imgJugadores = new JLabel [numeroJugadores];
		jugadores = new Jugador [numeroJugadores];
		for (int i = 0; i < numeroJugadores; i++) {
			imgJugadores[i] = new JLabel(new ImageIcon("src/main/java/img/jugador-"+(i+1)+".png"));
			jugadores[i] = new Jugador("", 1000);
		}
	...
	private void moverJugador(int resultadoDado[]) {
		Casilla aux = casillas.retornaPorPosicion((jugadores[jugadorActivo].getPosicion()+resultadoDado[0]+resultadoDado[1]));

		resultadoDado[2] = aux.getX();
		resultadoDado [3] = aux.getY();
		jugadores[jugadorActivo].setPosicionXY(resultadoDado [2],resultadoDado[3]);
		jugadores[jugadorActivo].setPosicion((jugadores[jugadorActivo].getPosicion()+resultadoDado[0]+resultadoDado[1]));
	}

	private void printInfoCasilla(int resultadoDado[]) {
		System.out.println("jugador: "+jugadorActivo+1);
